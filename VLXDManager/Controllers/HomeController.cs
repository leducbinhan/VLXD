...
namespace VLXDManager.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var vm = new DashboardViewModel // Đã sửa: thêm dấu ()
            {
                TongVatTu = await _context.VatTu.CountAsync(v => v.TrangThai), // Giả định TrangThai là bool, nếu là string có thể cần điều kiện khác
                VatTuSapHet = await _context.VatTu.CountAsync(v => v.TrangThai && v.SoLuongTon <= v.SoLuongMin), // Đã sửa: bổ sung điều kiện SoLuongTon
                TongKhachHang = await _context.KhachHang.CountAsync(k => k.TrangThai), // Giả định TrangThai là bool, nếu là string có thể cần điều kiện khác
